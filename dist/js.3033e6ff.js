...
function onClickToSelect(event) {
    _cards.cards.forEach(element => {
        if (event.target.id === element.name) {
            ...
            const markupSelectedCards = `<li class ="list" id = "${element.name}" style="background: center / contain no-repeat url('${element.img}')">
                <div>
                    <p></p>
                </div> 
            </li>`;
...
function onClickSort() {
    destroyCards();
    createMarkup(_cards.cards.sort(byField));
}
